...
- Shared compatible text handler orchestration for `/responses`, `/chat/completions`, and `/messages` now flows through `backend/internal/handler/compatible_gateway_text_flow.go`. Keep account selection, slot acquisition, failover, latency reporting, and usage recording in that shared helper instead of copying per-route loops.
