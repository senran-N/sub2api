...
- bulk-account payload state adaptation lives in `frontend/src/components/account/useBulkAccountMutationPayload.ts`; the bulk modal should pass batch apply/value refs there and keep submit handlers focused on no-selection validation, mixed-channel confirmation, and API bulk-update orchestration;
...
- `backend/internal/service/runtime_pipeline.go` is the native gateway pipeline for routes that need the full select -> account slot -> admission -> provider forward -> failover sequence. Route adapters pass protocol-specific hooks and render the returned `RuntimePipelineOutcome`; they should not own a second failed-account loop. Native Anthropic `/v1/messages`, OpenAI-compatible native Anthropic `/v1/chat/completions` and `/v1/responses`, plus Gemini native `/v1beta/models/*`, use this pipeline.
...
- Runtime setting dependencies should prefer the narrow facades in `backend/internal/service/setting_runtime_readers.go` (`GatewayRuntimeReader`, `GrokRuntimeReader`, `WebSearchRuntimeReader`, `AuthRuntimeReader`, `OpsRuntimeReader`) over injecting the full `SettingService` when the caller only needs runtime feature/config reads.
- Services started by Wire providers should be registered in `LifecycleRegistry` through `manageStartStopLifecycle` or `manageLifecycle`; provider functions should not call `Start()` without also registering `Stop()`.
