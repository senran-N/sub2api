...
import (
	...
	"github.com/senran-N/sub2api/internal/payment/provider"
...
func (s *PaymentService) CreateOrder(ctx context.Context, req CreateOrderRequest) (*CreateOrderResponse, error) {
	if err := s.applyWeChatResumeClaims(&req); err != nil {
		return nil, err
	}
	if req.OrderType == "" {
...
func (s *PaymentService) applyWeChatResumeClaims(req *CreateOrderRequest) error {
	if req == nil {
		return nil
	}
	...
	claims, err := s.paymentResume().ParseWeChatPaymentResumeToken(strings.TrimSpace(req.WeChatResumeToken))
	if err != nil {
		return err
	}
	...
	if normalized := NormalizeVisibleMethod(req.PaymentType); normalized == "" && claims.PaymentType != "" {
		req.PaymentType = claims.PaymentType
	}
	if req.Amount <= 0 && claims.Amount != "" {
		if amount, parseErr := strconv.ParseFloat(strings.TrimSpace(claims.Amount), 64); parseErr == nil {
			req.Amount = amount
		}
	}
	...
	if req.PlanID <= 0 && claims.PlanID > 0 {
		req.PlanID = claims.PlanID
	}
	if strings.TrimSpace(req.PaymentSource) == "" {
		req.PaymentSource = PaymentSourceWechatInAppResume
	}
	return nil
}
...
func (s *PaymentService) allocateOutTradeNo(ctx context.Context, tx *dbent.Tx) (string, error) {
	...
	return "", fmt.Errorf("generate unique out_trade_no: exhausted attempts")
}
...
func (s *PaymentService) prepareCreateOrderSelectionContext(ctx context.Context, req CreateOrderRequest) (context.Context, error) {
	...
	return payment.WithWxpayJSAPIAppID(ctx, appID), nil
}
...
func (s *PaymentService) getWeChatPaymentOAuthCredential(ctx context.Context) (string, string, error) {
	...
	appID, appSecret := wechatCredentialsForMode(cfg, "mp")
...
func buildCreateOrderResponse(order *dbent.PaymentOrder, req CreateOrderRequest, payAmount float64, sel *payment.InstanceSelection, pr *payment.CreatePaymentResponse, resultType payment.CreatePaymentResultType) *CreateOrderResponse {
	...
	resp := &CreateOrderResponse{
		...
		JSAPIParams:  pr.JSAPI,
		ExpiresAt:    order.ExpiresAt,
		PaymentMode:  paymentMode,
	}
	if pr.OAuth != nil {
		resp.RedirectURL = pr.OAuth.AuthorizeURL
	}
	return resp
}
...
func (s *PaymentService) GetUserOrders(ctx context.Context, userID int64, p OrderListParams) ([]*dbent.PaymentOrder, int, error) {
	...
	if p.Status != "" {
		query = query.Where(paymentorder.StatusEQ(p.Status))
	}
	if p.OrderType != "" {
		query = query.Where(paymentorder.OrderTypeEQ(p.OrderType))
	}
	if p.PaymentType != "" {
		query = query.Where(paymentorder.PaymentTypeEQ(p.PaymentType))
	}
	...
	orders, err := query.Order(dbent.Desc(paymentorder.FieldCreatedAt)).
		Limit(pageSize).
