...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_RewritesDefaultFallbackModelOnResponse(t *testing.T) {
	...
	account := &Account{
		ID:          116,
		Name:        "openai-ingress-default-map",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	err = clientConn.Write(writeCtx, coderws.MessageText, []byte(`{"type":"response.create","model":"gpt-5.4-xhigh","stream":false}`))
	cancelWrite()
	require.NoError(t, err)
	...
	require.Equal(t, "gpt-5.4-xhigh", gjson.GetBytes(firstEvent, "response.model").String())

	secondEvent := readMessage()
	require.Equal(t, "gpt-5.4-xhigh", gjson.GetBytes(secondEvent, "response.model").String())
