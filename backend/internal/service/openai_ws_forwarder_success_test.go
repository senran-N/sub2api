...
func TestOpenAIGatewayService_Forward_WSv2_OAuthStoreFalseByDefault(t *testing.T) {
	...
	require.Equal(t, isolateOpenAISessionID(account.ID, "sess-oauth-1"), captureDialer.lastHeaders.Get("session_id"))
	require.Equal(t, isolateOpenAISessionID(account.ID, "conv-oauth-1"), captureDialer.lastHeaders.Get("conversation_id"))
}
...
func TestOpenAIGatewayService_Forward_WSv2_HeaderSessionFallbackFromPromptCacheKey(t *testing.T) {
	...
	require.Equal(t, isolateOpenAISessionID(account.ID, "pcache_123"), captureDialer.lastHeaders.Get("session_id"))
...
func TestOpenAIGatewayService_Forward_WSv2_ConfiguredGeneratePrewarmDoesNotBlockRequestPath(t *testing.T) {
	...
	acquireLatency, ok := c.Get(OpsWSAcquireMsKey)
	require.True(t, ok)
	acquireLatencyValue, ok := acquireLatency.(int64)
	require.True(t, ok)
	require.GreaterOrEqual(t, acquireLatencyValue, int64(0))
	healthcheckLatency, ok := c.Get(OpsWSHealthcheckMsKey)
	require.True(t, ok)
	require.EqualValues(t, 0, healthcheckLatency)
