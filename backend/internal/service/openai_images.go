...
func walkOpenAIImageInlineAssets(node any, prompt string, out *[]openAIImagePointerInfo) {
	...
	case map[string]any:
		...
		item := openAIImagePointerInfo{
			Prompt:      localPrompt,
			Pointer:     firstNonEmptyImageString(value["asset_pointer"], value["pointer"]),
			DownloadURL: firstNonEmptyImageString(value["download_url"], value["url"], value["image_url"]),
			B64JSON:     firstNonEmptyImageString(value["b64_json"], value["base64"], value["image_base64"]),
			MimeType:    firstNonEmptyImageString(value["mime_type"], value["mimeType"], value["content_type"]),
		}
		switch {
...
func firstNonEmptyImageString(values ...any) string {
	for _, value := range values {
