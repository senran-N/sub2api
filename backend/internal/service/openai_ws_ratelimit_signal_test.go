...
func TestOpenAIGatewayService_Forward_OfficialCodexWSv2Handshake429TriggersFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	account := Account{
		ID:          504,
		Name:        "openai-ws-rate-limit-handshake-codex",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	require.False(t, c.Writer.Written(), "官方 Codex WS 429 应返回 failover 错误给上层换号，而不是直接写回客户端")
	require.Len(t, repo.rateLimitCalls, 1)
}
...
func TestOpenAIGatewayService_Forward_OfficialCodexWSv2Handshake403TriggersFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	account := Account{
		ID:          506,
		Name:        "openai-ws-auth-failed-handshake-codex",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	require.False(t, c.Writer.Written(), "官方 Codex WS 403 应返回 failover 错误给上层统一 exhaust，而不是直接写回客户端")
}
