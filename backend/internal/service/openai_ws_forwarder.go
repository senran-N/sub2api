...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	for {
		...
		if nextPayload.promptCacheKey != "" {
			...
			updatedHeaders, _ := s.buildOpenAIWSHeaders(c, account, token, ingressSession.wsDecision, ingressSession.isCodexCLI, ingressSession.turnState, strings.TrimSpace(c.GetHeader(openAIWSTurnMetadataHeader)), nextPayload.promptCacheKey)
