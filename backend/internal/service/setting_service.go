...
const (
	...
	openAIAdvancedSchedulerSettingErrorTTL  = 5 * time.Second
	openAIAdvancedSchedulerSettingDBTimeout = 5 * time.Second
)
...
func (s *SettingService) buildSystemSettingsUpdates(ctx context.Context, settings *SystemSettings) (map[string]string, error) {
	...
	updates[SettingKeyFrontendTheme] = strings.TrimSpace(settings.FrontendTheme)
...
func (s *SettingService) SetBetaPolicySettings(ctx context.Context, settings *BetaPolicySettings) error {
	...
	for i, rule := range settings.Rules {
		...
		// Normalize model_whitelist patterns; blank entries from UI forms are ignored.
		...
		for _, pattern := range rule.ModelWhitelist {
			...
			normalizedWhitelist = append(normalizedWhitelist, trimmed)
		}
		settings.Rules[i].ModelWhitelist = normalizedWhitelist
		settings.Rules[i].FallbackAction = strings.TrimSpace(rule.FallbackAction)
