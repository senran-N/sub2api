...
func TestOpenAIGatewayService_OAuthPassthrough_OfficialCodexPreservesMissingInstructionsBeforeUpstream(t *testing.T) {
	...
	originalBody := []byte(`{"model":"gpt-5.1-codex-max","stream":true,"store":true,"input":[{"type":"text","text":"hi"}]}`)

	upstreamSSE := strings.Join([]string{
		`data: {"type":"response.output_item.added","item":{"type":"message","content":[{"type":"output_text","text":"ok"}]}}`,
		"",
		"data: [DONE]",
		"",
	}, "\n")
...
func TestOpenAIGatewayService_OAuthPassthrough_ForceCodexCLICompatInjectsInstructionsBeforeUpstream(t *testing.T) {
	gin.SetMode(gin.TestMode)
...
func TestOpenAIGatewayService_APIKeyPassthrough_UsesAzureResponsesEndpointAndAPIKeyHeader(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	account := &Account{
		ID:             456,
		Name:           "azure-apikey-acc",
		Platform:       PlatformOpenAI,
		Type:           AccountTypeAPIKey,
		Concurrency:    1,
		Credentials:    map[string]any{"api_key": "azure-key", "base_url": "https://demo.cognitiveservices.azure.com/openai?api-version=2025-04-01-preview"},
		Extra:          map[string]any{"openai_passthrough": true},
		Status:         StatusActive,
		Schedulable:    true,
		RateMultiplier: f64p(1),
	...
	require.NotNil(t, result)
	require.NotNil(t, upstream.lastReq)
	require.Equal(t, "https://demo.cognitiveservices.azure.com/openai/responses?api-version=2025-04-01-preview", upstream.lastReq.URL.String())
	require.Equal(t, "azure-key", upstream.lastReq.Header.Get("Api-Key"))
	require.Empty(t, upstream.lastReq.Header.Get("Authorization"))
}

func TestOpenAIGatewayService_APIKeyPassthrough_ConvertsNonStreamingSSEToJSON(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	resp := &http.Response{
		...
		Body: io.NopCloser(strings.NewReader(strings.Join([]string{
			...
			`data: {"type":"response.completed","response":{"id":"resp_passthrough","model":"gpt-5.2","usage":{"input_tokens":3,"output_tokens":4}}}`,
			`data: [DONE]`,
		}, "\n"))),
	}
	upstream := &httpUpstreamRecorder{resp: resp}
	...
	require.Contains(t, rec.Body.String(), `"id":"resp_passthrough"`)
