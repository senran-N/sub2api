...
// ---------------------------------------------------------------------------
// NewRuntimeFailoverState 测试
// ---------------------------------------------------------------------------

func TestNewRuntimeFailoverState(t *testing.T) {
	t.Run("初始化字段正确", func(t *testing.T) {
		fs := NewRuntimeFailoverState(5, true)
...
func TestSleepWithContext(t *testing.T) {
	t.Run("零时长立即返回nil", func(t *testing.T) {
		start := time.Now()
	...
	t.Run("负时长立即返回nil", func(t *testing.T) {
		start := time.Now()
	...
	t.Run("正常等待后返回nil", func(t *testing.T) {
		start := time.Now()
	...
	t.Run("已取消context立即返回错误", func(t *testing.T) {
	...
	t.Run("等待期间context取消返回错误", func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		go func() {
...
func TestHandleFailoverError_BasicSwitch(t *testing.T) {
	t.Run("非重试错误_非Antigravity_直接切换", func(t *testing.T) {
		...
		require.Equal(t, RuntimeFailoverContinue, action)
	...
	t.Run("非重试错误_Antigravity_第一次切换无延迟", func(t *testing.T) {
		...
		start := time.Now()
		action := fs.HandleFailoverError(context.Background(), mock, 100, PlatformAntigravity, err)
		elapsed := time.Since(start)
	...
	t.Run("非重试错误_Antigravity_第二次切换有1秒延迟", func(t *testing.T) {
		...
		err := newTestFailoverErr(500, false, false)
		start := time.Now()
		action := fs.HandleFailoverError(context.Background(), mock, 200, PlatformAntigravity, err)
		elapsed := time.Since(start)
	...
	t.Run("连续切换直到耗尽", func(t *testing.T) {
		mock := &mockTempUnscheduler{}
		fs := NewRuntimeFailoverState(2, false)
		...
		require.Equal(t, RuntimeFailoverExhausted, action)
...
func TestHandleFailoverError_CacheBilling(t *testing.T) {
	t.Run("hasBoundSession为true时设置ForceCacheBilling", func(t *testing.T) {
		mock := &mockTempUnscheduler{}
		fs := NewRuntimeFailoverState(3, true) // hasBoundSession=true
		err := newTestFailoverErr(500, false, false)
...
func TestHandleFailoverError_SameAccountRetry(t *testing.T) {
	t.Run("第一次重试返回RuntimeFailoverContinue", func(t *testing.T) {
		mock := &mockTempUnscheduler{}
		fs := NewRuntimeFailoverState(3, false)
	...
	t.Run("bound session 429 retries then exhausts without switching", func(t *testing.T) {
		mock := &mockTempUnscheduler{}
		fs := NewRuntimeFailoverState(3, true)
	...
	t.Run("达到最大重试次数前均返回RuntimeFailoverContinue", func(t *testing.T) {
		mock := &mockTempUnscheduler{}
		fs := NewRuntimeFailoverState(3, false)
		err := newTestFailoverErr(400, true, false)
	...
	t.Run("不同账号独立跟踪重试次数", func(t *testing.T) {
		mock := &mockTempUnscheduler{}
		fs := NewRuntimeFailoverState(5, false)
		err := newTestFailoverErr(400, true, false)
...
func TestHandleFailoverError_ContextCanceled(t *testing.T) {
	t.Run("同账号重试sleep期间context取消", func(t *testing.T) {
		...
		require.Equal(t, RuntimeFailoverCanceled, action)
	...
	t.Run("Antigravity延迟期间context取消", func(t *testing.T) {
		...
		start := time.Now()
		action := fs.HandleFailoverError(ctx, mock, 100, PlatformAntigravity, err)
		elapsed := time.Since(start)
...
func TestHandleFailoverError_IntegrationScenario(t *testing.T) {
	...
	t.Run("模拟Antigravity平台完整流程", func(t *testing.T) {
		...
		start = time.Now()
		action = fs.HandleFailoverError(context.Background(), mock, 300, PlatformAntigravity, err)
		elapsed = time.Since(start)
		require.Equal(t, RuntimeFailoverExhausted, action)
	...
	t.Run("ForceCacheBilling通过错误标志设置", func(t *testing.T) {
		mock := &mockTempUnscheduler{}
		fs := NewRuntimeFailoverState(3, false) // hasBoundSession=false
