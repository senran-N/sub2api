...
func (h *AuthHandler) OIDCOAuthCallback(c *gin.Context) {
	...
	if existingIdentityUser != nil {
		...
		_ = h.authService.BindOAuthIdentity(c.Request.Context(), serviceUser.ID, "oidc", issuer, subject, map[string]any{
			"email":        email,
			"compat_email": compatEmail,
			"username":     username,
			"issuer":       issuer,
			"source":       "oidc_oauth_identity_login",
		})
	...
	choiceEmail := strings.TrimSpace(compatEmail)
