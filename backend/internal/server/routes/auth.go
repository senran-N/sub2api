...
import (
	...
	"github.com/senran-N/sub2api/internal/middleware"
...
func RegisterAuthRoutes(
	...
	auth.Use(servermiddleware.BackendModeAuthGuard(settingService))
	{
		...
		auth.POST("/oauth/linuxdo/bind-login",
			rateLimiter.LimitWithOptions("oauth-linuxdo-bind-login", 10, time.Minute, middleware.RateLimitOptions{
				FailureMode: middleware.RateLimitFailClose,
			}),
		...
		auth.POST("/oauth/oidc/bind-login",
			rateLimiter.LimitWithOptions("oauth-oidc-bind-login", 10, time.Minute, middleware.RateLimitOptions{
				FailureMode: middleware.RateLimitFailClose,
			}),
