...
describe('usagePresentation helpers', () => {
  ...
  it('derives legacy request types and status badge classes', () => {
    ...
    expect(getUsageRequestTypeBadgeClass({ request_type: 'stream' })).toContain('theme-chip--info')
    expect(getUsageRequestTypeBadgeClass({ request_type: 'sync' })).toContain('theme-chip--neutral')
    expect(getUsageRequestTypeBadgeClass({ request_type: 'ws_v2' })).toContain('theme-chip--brand-purple')
    expect(getUsageRequestTypeBadgeClass({ request_type: 'unknown' })).toContain('theme-chip--warning')
  })
})
