...
import { getLocale, translateMessage } from "@/i18n";
...
export function formatRelativeTime(
  ...
  if (!date) return translateMessage("common.time.never");
  ...
  // 处理未来时间或无效日期
  if (diffMs < 0 || isNaN(diffMs)) return translateMessage("common.time.never");
  ...
  if (diffDays > 0)
    return translateMessage("common.time.daysAgo", { n: diffDays });
  if (diffHours > 0)
    return translateMessage("common.time.hoursAgo", { n: diffHours });
  if (diffMins > 0)
    return translateMessage("common.time.minutesAgo", { n: diffMins });
  return translateMessage("common.time.justNow");
}
...
export function formatCountdown(
  ...
  if (diffDays > 0) {
    // 超过1天：显示 "Xd Yh"
    return translateMessage("common.time.countdown.daysHours", {
      d: diffDays,
      h: remainingHours,
    });
  }
  if (diffHours > 0) {
    // 小于1天：显示 "Xh Ym"
    return translateMessage("common.time.countdown.hoursMinutes", {
      h: diffHours,
      m: remainingMins,
    });
  }
  // 小于1小时：显示 "Ym"
  return translateMessage("common.time.countdown.minutes", { m: diffMins });
}
...
export function formatCountdownWithSuffix(
  ...
  return translateMessage("common.time.countdown.withSuffix", {
    time: countdown,
  });
}
...
export function formatRelativeWithDateTime(
  ...
  if (!dateTime || relativeTime === translateMessage("common.time.never")) {
    return relativeTime;
  }
