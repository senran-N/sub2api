...
import { getThemeChartSequence, readThemeCssVariable } from '@/utils/themeStyles'
...
describe('ModelDistributionChart', () => {
  ...
  it('renders Others in the spending ranking table and uses a dedicated chart color', async () => {
    ...
    expect(chartData.datasets[0].backgroundColor[0]).toBe(getThemeChartSequence()[0])
    expect(chartData.datasets[0].backgroundColor[2]).toBe(readThemeCssVariable('--theme-page-muted'))
