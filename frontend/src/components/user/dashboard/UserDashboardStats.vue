    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div class="min-w-0">
          ...
          <p class="truncate text-xs text-gray-500 dark:text-gray-400" :title="`${t('dashboard.input')}: ${formatTokens(stats?.today_input_tokens || 0)} / ${t('dashboard.output')}: ${formatTokens(stats?.today_output_tokens || 0)}`">{{ t('dashboard.input') }}: {{ formatTokens(stats?.today_input_tokens || 0) }} / {{ t('dashboard.output') }}: {{ formatTokens(stats?.today_output_tokens || 0) }}</p>
        </div>
      </div>
    </div>
