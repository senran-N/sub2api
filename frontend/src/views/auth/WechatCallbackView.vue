    ...
    <div class="space-y-6">
      <div class="text-center">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">
          {{ t('auth.wechat.callbackTitle', { providerName }) }}
        </h2>
        <p class="mt-2 text-sm text-gray-500 dark:text-dark-400">
          {{
            isProcessing
              ? t('auth.wechat.callbackProcessing', { providerName })
              : t('auth.wechat.callbackHint')
          }}
        </p>
      </div>
        ...
        >
          ...
          <template v-if="needsInvitation">
            <p class="text-sm text-gray-700 dark:text-gray-300">
              {{ t('auth.wechat.invitationRequired', { providerName }) }}
            </p>
            <div>
              <input
            ...
            >
              {{
                isSubmitting
                  ? t('auth.wechat.completing')
                  : t('auth.wechat.completeRegistration')
              }}
            </button>
          </template>
...
const lastCreateAccountPayload = ref<PendingOAuthCreateAccountPayload | null>(null)
...
async function loadProviderName() {
  providerName.value = t('auth.wechatProviderName')
  try {
    await getPublicSettings()
  } catch {
    // Ignore; fallback remains WeChat
  }
}
...
async function finalizeCompletion(completion: PendingOAuthExchangeResponse, redirect: string) {
  ...
  if (!isOAuthLoginCompletion(completion)) {
    throw new Error(t('auth.wechat.callbackMissingToken'))
  }
...
async function handleSubmitInvitation() {
  ...
  const payload = lastCreateAccountPayload.value
  if (!invitationCode.value.trim() || !payload?.email || !payload.password) return
  ...
  try {
    ...
    invitationError.value = getRequestErrorMessage(e, t('auth.wechat.completeRegistrationFailed'))
  } finally {
...
async function handleCreateAccount(payload: PendingOAuthCreateAccountPayload) {
  ...
  isSubmitting.value = true
  try {
    lastCreateAccountPayload.value = payload
    const { data } = await apiClient.post<PendingWechatCompletion>('/auth/oauth/pending/create-account', {
      email: payload.email,
      password: payload.password,
      verify_code: payload.verifyCode || undefined,
