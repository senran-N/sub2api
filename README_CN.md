...
- **Claude Code 运行时伪装同步** - 服务启动时自动从 npm 拉取最新 Claude Code 包，并按周期刷新伪装请求头与提示词
...
- `server.shutdown_timeout_seconds` 控制优雅关停时长
...
Sub2API 默认会在服务启动时从 npm 拉取最新 `@anthropic-ai/claude-code` 发布包，并按周期刷新 Claude Code 反代使用的运行时伪装画像。这样 `x-app`、beta 头、prompt 前缀以及基于版本生成的 `User-Agent` 都会跟随上游版本更新，不需要手工改代码。

相关配置见 [`deploy/config.example.yaml`](deploy/config.example.yaml)：
